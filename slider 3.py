...
def psi_plotter():
    ...
    try:
        print("Please input your V(x)")
        print("You may use numpy functions if you wish.")
        V_x = input("")
        
        def my_func(Nx=500, xmin=-5, xmax=5, tmin=0, tmax=20, V_x=V_x):
            ...
            return x, t_array, V_x, psi_list


        # Calls return arrays in my_func
        x, t_array, V_x, psi_list = my_func()

        # Saves key data into a pandas data frame
        df = pd.DataFrame({'x': x, 'time': t_array,
                        'V(x)': V_x, 'psi': psi_list})
        ...
        fig, ax = plt.subplots()
        line, = ax.plot(x, psi_list[0], lw=2)
        ...
        # Creaes plot for V_x
        ax_twin = ax.twinx()
        line_2, = ax_twin.plot(x, V_x, color="C1")
        ...
        def update(val):
            ...
            x, t_array, V_x, psi_list = my_func(Nx=int(NxNt_slider.val))
